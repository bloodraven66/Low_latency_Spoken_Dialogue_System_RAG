...
from unmute.unmute_handler_speculative_rag_online import UnmuteHandlerSpeculativeRAGOnline

RAG_SERVER = "http://127.0.0.1:8095"
...
@app.get("/")
def root():
    return {"message": "You've reached the Unmute RAG backend server."}
...
class HealthStatus(BaseModel):
    ...
    @property
    def ok(self) -> bool:
        # rag_up and anticipator_up are advisory — handler degrades gracefully if either is down.
...
async def _get_health(
    ...
    async with asyncio.TaskGroup() as tg:
        ...
        rag_up = tg.create_task(
            asyncio.to_thread(
                _check_server_status, RAG_SERVER + "/api/health"
            )
        )
        anticipator_up = tg.create_task(
            asyncio.to_thread(
                _check_server_status, _ws_to_http(ENDPOINTER_SERVER) + "/api/build_info"
            )
        )
        ...
        rag_up_res = await rag_up
        anticipator_up_res = await anticipator_up
    ...
    return HealthStatus(
        ...
        llm_up=llm_up_res,
        rag_up=rag_up_res,
        anticipator_up=anticipator_up_res,
...
async def websocket_route(websocket: WebSocket):
    ...
    async with SEMAPHORE:
        try:
            ...
            handler = UnmuteHandlerSpeculativeRAGOnline(rag_top_k=1)
            async with handler:
