...
from unmute.unmute_handler_rag import UnmuteHandlerRAG
...
class HealthStatus(BaseModel):
    ...
    @property
    def ok(self) -> bool:
        # rag_up is advisory — handler falls back to baseline if RAG is down.
...
async def websocket_route(websocket: WebSocket):
    ...
    async with SEMAPHORE:
        try:
            ...
            handler = UnmuteHandlerRAG(rag_top_k=1)
            async with handler:
