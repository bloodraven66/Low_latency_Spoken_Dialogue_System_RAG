...
_DEFAULT_ADDITIONAL_INSTRUCTIONS_WITHOUT_STARTER = """
There should be a lot of back and forth between you and the other person.
Ask follow-up questions etc.
Don't be servile. Be a good conversationalist, but don't be afraid to disagree, or be
a bit snarky if appropriate.
...
SMALLTALK_INSTRUCTIONS_WITHOUT_STARTER = """
{additional_instructions}
...
Repond to the user's message with an engaging reply.
"""
...
#             ),
#         )
#         a = _SYSTEM_PROMPT_TEMPLATE.format(
#             _SYSTEM_PROMPT_BASICS=_SYSTEM_PROMPT_BASICS,
#             additional_instructions=additional_instructions,
#             language_instructions=LANGUAGE_CODE_TO_INSTRUCTIONS[self.language],
#             llm_name=get_readable_llm_name(),
...
class SmalltalkInstructions(BaseModel):
    # type: Literal["smalltalk_no_starter"] = "smalltalk_no_starter"
    type: Literal["smalltalk"] = "smalltalk"
    language: LanguageCode | None = None
    def make_system_prompt(
        self,
        additional_instructions: str = _DEFAULT_ADDITIONAL_INSTRUCTIONS_WITHOUT_STARTER,
    ) -> str:
        additional_instructions = SMALLTALK_INSTRUCTIONS_WITHOUT_STARTER.format(
            additional_instructions=additional_instructions,
            current_time=datetime.datetime.now().strftime("%A, %B %d, %Y at %H:%M"),
            timezone=datetime.datetime.now().astimezone().tzname(),
        )
        
        # a = _SYSTEM_PROMPT_TEMPLATE_SHORT.format(
        #     _SYSTEM_PROMPT_BASICS=_SYSTEM_PROMPT_BASICS,
        #     additional_instructions=additional_instructions,
        #     language_instructions=LANGUAGE_CODE_TO_INSTRUCTIONS[self.language],
        #     llm_name=get_readable_llm_name(),
        ...
        return _SYSTEM_PROMPT_TEMPLATE_SHORT.format(
            _SYSTEM_PROMPT_BASICS=_SYSTEM_PROMPT_BASICS,
            additional_instructions=additional_instructions,
            language_instructions=LANGUAGE_CODE_TO_INSTRUCTIONS[self.language],
            llm_name=get_readable_llm_name(),
...
# YOUR ROLE
You are a voice assistant made by Kyutai.
Answer the user's questions directly and factually.
...
class BaselineInstructions(BaseModel):
